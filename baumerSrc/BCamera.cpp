...
namespace baumer {
  ...
  void
  BCamera::setSaveFrames(bool truefalse){
    m_save_frames = truefalse;
  }
  ...
  void
  BCamera::saveFrame(const char* filename){
    static unsigned int counter = 0;

    png::image< png::rgb_pixel > output(getWidth(), getHeight());
	...
	p.red = m_front[buffid++];
	p.green = m_front[buffid++];
	p.blue = m_front[buffid++];
    ...
    if(NULL == filename)
      output.write(std::string("captured_frames/tmp_frame_" + toString(this) + "_" + toString(counter++) + ".png"));
    else
      output.write(std::string(filename) + ".png");
